...
public class Main {
    ...
    static {
        whitePlayer = new ConsolePlayer("White", Color.YELLOW);
        blackPlayer = new ComputerPlayer("Black", Color.BLUE, Strategy.SMART);
        size = 5;
    ...
    public static Game createGame(int size, int numMovesProtection, int criticalRegionSize,
        ...
        Piece[] ps = SmartPlayerFactory.generateSmartPieces(size, blackPlayer);
        for (int i = 0; i < size; i++) {
            if (i % 2 == 0) {
                var piece = new Knight(blackPlayer);
                configuration.addInitialPiece(/*piece*/ps[i], i, size - 1);
            } else {
