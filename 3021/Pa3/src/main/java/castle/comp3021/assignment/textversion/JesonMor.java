...
public class JesonMor extends Game {
    ...
    private LimitedStack<MoveDetails> moveStack = new LimitedStack<>(999);

    private class MoveDetails {
        private final Move move;
        private final Player movedBy;
        private final int scoreAtTheMoment;
        private final Piece captured;

        public MoveDetails(Move move, Player player, int score, Piece captured) {
            this.move = move; // revert the move
            this.movedBy = player;
            this.scoreAtTheMoment = score;
        ...
        public Move getRevertedMove() {
            return new Move(move.getDestination(), move.getSource());
        }
        public Player getWhoMoved() {
            return movedBy;
        }
        public int getScoreAtTheMoment() {
            return scoreAtTheMoment;
        }
    ...
    public Player start() {
        ...
        while (true) {
            ...
            if (availableMoves.length <= 0) {
                ...
                Piece cap = getPiece(move.getDestination());
                if (cap != null && cap.getPlayer() instanceof ComputerPlayer) {
                    cap.terminate();
                }
                moveStack.push(new MoveDetails(move, player, player.getScore(), cap));
            ...
            for (int j = 0; j < configuration.getSize(); j++) {
                if (board[i][j] != null && board[i][j].getPlayer().equals(player)) {
                    if (player instanceof HumanPlayer){
                        Collections.addAll(moves, board[i][j].getAvailableMoves(this, new Place(i, j)));
                    } else {
                        System.out.println("Computer is figuring out next move...");
                        Collections.addAll(moves, board[i][j].getCandidateMove(this, new Place(i, j)));
                    }
                }
            }
        }
        return moves.stream().filter(Objects::nonNull).toArray(Move[]::new);
    }
    ...
    public void undo() throws UndoException {
        ...
        if (!(ps[0] instanceof HumanPlayer && ps[1] instanceof ComputerPlayer ||
           (ps[1] instanceof HumanPlayer && ps[0] instanceof ComputerPlayer))) {
        ...
        if (Game.undoLimit == 0) {
            throw new UndoException("Undo limit is reached");
        }
        for (int i = 0; i < 2; i++) {
            undoMove();
        }
    ...
    private void undoMove() {
        MoveDetails md = moveStack.pop();
        Move rev = md.getRevertedMove();
        movePiece(rev);
        if (md.getCaptured() != null) {
            board[rev.getSource().x()][rev.getSource().y()] = md.getCaptured();
        }
        Player p = md.getWhoMoved();
        p.setScore(md.getScoreAtTheMoment());
        moveRecords.removeIf(mr -> mr.getMove().equals(md.getMove()) && mr.getPlayer().equals(p));
        numMoves--;
    }
